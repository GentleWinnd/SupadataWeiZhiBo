...
@interface ControlScreen ()<AVPlayerViewControllerDelegate, ZYLButtonDelegate>
...
@property (strong, nonatomic) ScreenBtnControl *button;
...
- (void)viewDidLoad {
    ...
    playerLayer.frame = CGRectMake(0, 0, 0,0);
...
- (void)addScreenControlFrame:(CGRect) frame addView:(UIView *)view {

    self.button = [[ScreenBtnControl alloc] initWithFrame:frame];
    self.button.touchDelegate = self;
    [view insertSubview:self.button atIndex:0];
//    self.volumeView.frame = CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.width * 9.0 / 16.0);
