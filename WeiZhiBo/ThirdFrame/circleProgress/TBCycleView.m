...
- (void)drawRect:(CGRect)rect {
    ...
    CGPoint center = CGPointMake(56, 56);//设置圆心位置
    CGFloat radius = 54;//设置半径
    CGFloat startA = 0;//圆起点位置
    CGFloat endA = M_PI*4;//圆终点位置
    UIBezierPath *path = [UIBezierPath bezierPathWithArcCenter:center radius:radius startAngle:startA endAngle:endA clockwise:YES]; CGContextSetLineWidth(ctx, 3);//设置线条宽度
    [CIRCLE_LIGHTBLUE_OUT setStroke];//设置描边颜色
...
- (void)drawCycleProgress
{
    ...
    CGFloat radius = 52;
    CGFloat startA = -M_PI;  //设置进度条起点位置
    CGFloat endA = -M_PI + M_PI * 2 * _progress;  //设置进度条终点位置
    ...
    _progressLayer.lineWidth = 8;//线的宽度
